...
## Models trained with only scanner data
### K-Means Clustering
```{r,warning=FALSE}
...
set.seed(47)
goodind<-sample(which(bigdf$cluster=="good"),3,replace=FALSE)
badind<-sample(which(bigdf$cluster=="bad"),3,replace=FALSE)
...
train_df<-data.frame(cbind(scanner_pca_train,bigdf[-testind,]))
train_df <- train_df %>%
  select(-profile_resid,-texture)

test_df<-data.frame(cbind(scanner_pca_test,bigdf[testind,]))
test_df <- test_df %>%
  select(-profile_resid,-texture)
```
...
library(Metrics)
control <- trainControl(method="oob",verboseIter = FALSE,sampling = "up")
seed <- 47
metric <- "Accuracy"
set.seed(seed)
mtry <- 1:7
...
train_mat_pca<-mat_pca[-testind,]

for (i in 1:dim(train_mat_pca)[1]){
  mat<-matrix(train_mat_pca[i,],1520,1628);
  write.csv(mat,paste("umpca_reconstruction/matrices",namelst[i],".csv",sep=''))
}
```
...
rf_vr
confusionMatrix(predict(rf_vr,vr_test_df),bigdf[testind,]$cluster)
```
