...
#testind<-sample(nrow(bigdf),6,replace = FALSE)
bigdf[testind,]
```
...
file.remove(file.path("umpca_reconstruction/matrices/train/", list.files("umpca_reconstruction/matrices/train/"))) 
for (i in trainind){
  mat<-matrix(mat_pca[i,],1520,1628);
  write.csv(mat,paste("umpca_reconstruction/matrices/train/",namelst[i],".csv",sep=''))
}

file.remove(file.path("umpca_reconstruction/matrices/test/", list.files("umpca_reconstruction/matrices/test/"))) 
for (i in testind){
  mat<-matrix(mat_pca[i,],1520,1628);
