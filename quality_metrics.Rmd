...
plot(xRef,zRef,col="orange",cex=0.2,xlab="X (um)",ylab="Z (um)",main = "Before Alignment",asp=1)
points(profile_lst[[4*12+1]],profile_lst[[4*12+2]],cex=0.2,col="blue",asp=1)
...
plot(alignmentOBE,type="three",off=1,main="Matching",asp=0.05)
```
...
#  matchdata<-alignment$index1[which(alignment$index2==which(zRef==max(zRef)))] 
#  matchref<-alignment$index2[which(alignment$index2==which(zRef==max(zRef)))]
...
```{r}
# plots that check alignment 
j<-1 # which profile
p<-7# which sample
ndf<-(j-1)/2
ref1_ind<-3*12+j
myPlot<-function(index) {points(y=bigdf[[index+1]],x=bigdf[[index]],cex=0.2,col="blue")}
plot(x=profile_lst[[ref1_ind]],y=profile_lst[[ref1_ind+1]],cex=0.2,col="orange",asp=1,,xlab="X (um)",ylab="Z (um)",main="Aligned Curves")
